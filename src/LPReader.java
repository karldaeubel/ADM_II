 ...
 *
 * Initialise with the file name and invoke the method <code>readLP()</code>
 * to parse an lp from an <code>.lp</code> file.
 * After successful reading all data is held in various arrays
 * which can be accessed via class methods.
 * Debug output can be switched on by an additional parameter to the constructor.
 * <br/>
...
public class LPReader
{
	 ...
	 return constrName.length;
       ...
       *
       * @return the lhs coefficients of all constraints as an array of size <code>noOfConstraints()</code> &times; <code>noOfVariables()</code>
       */
	    ...
	    if (line.length() > 0)
	    {
		  ...
		  switch (status)
		  {
			...
			status = SEC_OBJECTIVE;
		     ...
		     case SEC_OBJECTIVE:
		     {
			if (keyword.equals("subject to") || keyword.equals("such that") ||
			    keyword.equals("s.t.") || keyword.equals("st.") || keyword.equals("st"))
			{
			...
			else
			{
			   ...
			   if (constrComplete)
			   {
				 ...
				 throw new ParseException ("line "+lineNo+": ambiguous constraint name '"+curConstr.name+addInfo, lineNo);
			      }
	 ...
	 hash2arr ();
      ...
      private void parseBound (String expr, int lineNo)
	 ...
	 switch (exprsplit.length)
	 {
	    ...
	    case 2:
	    {
		  ...
		  if (var == null)
		     throw new ParseException ("line "+lineNo+": neither '"+boundStr+"' nor '"+varname+"' is a known variable", lineNo);
	       }
	       double bound = 0;
	       try
	       {
	    ...
	    case 3:
	    {
		  ...
		  catch (NumberFormatException ex)
		  {
	       ...
	       if (upperStr.equals("infinity") || upperStr.equals("inf")
		   || upperStr.equals("+infinity") || upperStr.equals("+inf"))
